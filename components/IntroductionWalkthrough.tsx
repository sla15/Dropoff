  ...
  return (
    <div className={`fixed inset-0 z-[10000] overflow-hidden transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div 
        ...
        className={`absolute ${bgCard} rounded-[28px] px-5 py-5 shadow-2xl z-10 transition-all duration-500 ease-in-out flex flex-col items-center text-center overflow-hidden`}
      >
        <button 
          onClick={skip}
          className="absolute top-4 right-4 p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors"
        >
          <X size={18} className={textSec} />
        </button>

        <div className="flex flex-col items-center text-center mt-1 w-full">
          {steps[currentStep].icon}
          
          <h3 className={`text-[17px] font-black mb-2 ${textMain} tracking-tight leading-tight`}>
            {steps[currentStep].title}
          </h3>
          
          <p className={`text-[13px] font-medium leading-relaxed ${textSec} mb-5`}>
            ...
            {steps.map((_, i) => (
              <div 
                ...
                className={`h-1 rounded-full transition-all duration-300 ${
                  i === currentStep ? 'w-6 bg-[#00D68F]' : 'w-1.5 bg-[#00D68F]/20'
                }`}
              />
            ))}
          </div>
          ...
          <div className="flex w-full gap-2">
            ...
            <button 
              onClick={next}
              className="flex-[2] py-3 bg-[#00D68F] text-black rounded-2xl text-sm font-bold flex items-center justify-center gap-1.5 active:scale-95 transition-all"
            >
